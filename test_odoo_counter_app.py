...
def _download_invoice_pdf(models, uid, invoice_id: int, invoice_name: str) -> Path | None:
    """Web-session login + /report/pdf download, mirroring fetch_odoo_pdf.py. None on any failure."""
    try:
...
class BarcodeWorker(QThread):
    ...
    invoice_pdf_ready    = pyqtSignal(str)  # fire เมื่อดาวน์โหลด invoice PDF สำเร็จ (demo KAN-70)
    invoice_pdf_fetching = pyqtSignal(str)  # fire ก่อนเริ่มดาวน์โหลดจริง (ผ่าน gate ครบแล้ว) — ส่งเลขที่ใบเสร็จ
    invoice_pdf_failed   = pyqtSignal(str)  # fire เมื่อดาวน์โหลดล้มเหลว — ส่งเลขที่ใบเสร็จ
    ...
    def _create_and_post_invoice(self, models, uid, sale_order_id: int) -> list | None:
        ...
        Odoo's standard "Create Invoice" wizard (sale.advance.payment.inv, same as the UI button)
        and post it immediately so it has a real tax invoice number (see CONTEXT.md 'Auto-invoice
        creation' and docs/adr/0003). Returns new invoice_ids, or None on failure.

        sale.order._create_invoices() itself can't be used — Odoo's XML-RPC layer hard-rejects any
        call to a leading-underscore method ("Private methods ... cannot be called remotely.").
        The wizard is the public entry point the "Create Invoice" button actually goes through.

        The test tenant's XML-RPC layer occasionally raises "cannot marshal None" on the response
        of create_invoices/action_post even though the write already succeeded server-side — see
        _run_create_invoices_with_recovery / _post_invoices_with_recovery.
        """
        self.invoice_creating.emit()
        try:
            ...
            new_invoice_ids = self._run_create_invoices_with_recovery(models, uid, sale_order_id, wizard_id, ctx)
    ...
    def _post_invoices_with_recovery(self, models, uid, invoice_ids: list):
    ...
    def _fetch_invoice_pdf(self, models, uid, sale_order_id: int):
        """Demo (KAN-70): gate on x_studio_need_bill, resolve via sale_id -> invoice_ids only."""
        orders = models.execute_kw(
            ODOO_DB, uid, ODOO_PASSWORD,
            'sale.order', 'read',
            [[sale_order_id]],
            {'fields': ['x_studio_need_bill', 'invoice_ids']}
        )
        if not orders:
            return
        need_bill_field = orders[0].get('x_studio_need_bill')
        # Odoo Studio field is many2one-shaped ([id, display_name]), not a plain string
        need_bill = need_bill_field[1] if isinstance(need_bill_field, (list, tuple)) else (need_bill_field or '')
        if (need_bill or '').strip() != INVOICE_NEED_BILL_VALUE:
            return
        invoice_ids = orders[0].get('invoice_ids') or []
        if not invoice_ids:
            # ยังไม่เคยมี invoice เลย (ไม่ใช่ draft/cancel ค้าง) — สร้างให้อัตโนมัติแล้วไปต่อ
            invoice_ids = self._create_and_post_invoice(models, uid, sale_order_id)
            if not invoice_ids:
                return
        ...
        self.invoice_pdf_fetching.emit(invoice_name)
        path = _download_invoice_pdf(models, uid, invoices[0]['id'], invoice_name)
        if path:
...
class MainWindow(QMainWindow):
    ...
    def _on_invoice_creating(self):
        ...
        self.lbl_status.setStyleSheet("color:#888; font-size:12px;")
        self.lbl_status.setText("⏳ กำลังออกใบกำกับภาษี — กรุณารอสักครู่...")

    def _on_invoice_create_unblock(self):
        # คืนสถานะสแกนตามที่ควรจะเป็นจริง ๆ — ถ้าหน้านับยังเปิดอยู่ต้องยังบล็อกต่อ ไม่ใช่ปลดล็อกมั่ว
        self._barcode_listener.set_active(not self._counter_panel.isVisible())

    def _on_invoice_create_failed(self, msg: str):
        self.lbl_status.setStyleSheet("color:#EF9A9A; font-size:12px;")
        self.lbl_status.setText("สร้างใบกำกับภาษีไม่สำเร็จ — กรุณาสร้างเองใน Odoo (ดู console)")

    def _on_invoice_pdf_fetching(self, invoice_name: str):
        self.lbl_status.setStyleSheet("color:#888; font-size:12px;")
        self.lbl_status.setText(f"กำลังดึงใบเสร็จ {invoice_name} ...")

    @staticmethod
    def _on_invoice_pdf_ready(path: str):
        try:
            os.startfile(path)
        except Exception as e:
            print(f"[Invoice] เปิดไฟล์ไม่ได้: {e}", flush=True)

    def _on_invoice_pdf_failed(self, invoice_name: str):
        self.lbl_status.setStyleSheet("color:#EF9A9A; font-size:12px;")
        self.lbl_status.setText(f"ดึงใบเสร็จ {invoice_name} ไม่สำเร็จ — ดูรายละเอียดใน console")
