...
- **`_apply_thai_baht_wording()` must not call `ir.actions.server.run()` again.** The hardcoded `operation.engineer@tdfb.co` account has no access to that model in production (Settings/Administration-only, by Odoo design) — confirmed live, and it isn't a scoped permission gap you can fix with a narrower grant. It computes the Thai baht-text locally now (`_thai_number_to_text`/`_amount_to_thai_baht_text` in `odoo_counter_app.py`). See `docs/superpowers/specs/2026-08-10-thai-baht-text-local-computation-design.md`.
